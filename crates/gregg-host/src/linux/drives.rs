...
use crate::drives::{normalize_with_limits, DriveCandidate};
use crate::error::CollectError;
use crate::linux::source::{ProcSource, RawStatvfs};
...
/// Collect drives using explicit limits.
pub fn collect_with_limits(
    source: &ProcSource,
    limits: &CollectionLimits,
) -> Result<Vec<DriveMetrics>, CollectError> {
    ...
    Ok(normalize_with_limits(candidates, limits))
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::linux::source::{MemorySource, RawStatvfs};
    use std::sync::Arc;
    ...
    fn parses_filters_deduplicates_and_sorts() {
        ...
        let drives = collect_with_limits(&source, &CollectionLimits::gregg_defaults()).unwrap();
        assert_eq!(
            drives
                .iter()
    ...
    fn preserves_total_free_and_caller_available_separately() {
        ...
        let drive = collect_with_limits(&source, &CollectionLimits::gregg_defaults())
            .unwrap()
            .into_iter()
            .next()
            .unwrap();
