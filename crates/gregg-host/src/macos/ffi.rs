...
mod native_tests {
    ...
    fn native_collector_publishes_v2_drives_and_network() {
        ...
        let metrics = loop {
            ...
            let metrics = match collector.sample() {
                ...
                Err(error) => panic!("native warmup sample fails: {error:?}"),
            };
            let drives_ready = metrics
                .drives
                .as_ref()
                .is_some_and(|drives| !drives.is_empty());
            // Require a non-loopback interface in the complete sample, not
            ...
            let network_ready = metrics.network.as_ref().is_some_and(|network| {
            ...
            if elapsed >= deadline {
                panic!(
                    "bounded warmup expired without drives+non-loopback network: drives={:?} network_interfaces={:?}",
                    metrics.drives.as_ref().map(Vec::len),
                    metrics.network.as_ref().map(|network| network
                        .interfaces
                        .iter()
        ...
        let network = metrics.network.as_ref().expect("network present");
        assert!(
            network
                .interfaces
                .iter()
                .any(|interface| !interface.is_loopback),
            "sample must contain at least one non-loopback interface"
        );
        // Disk I/O stays optional: hosted/virtualized Macs may genuinely
        // expose no usable IOKit block-driver counters.
        let _ = metrics.disk_io;
        // Frequency remains unavailable on supported macOS sources.
    ...
    fn complete_production_collector_smoke() {
        ...
        // 6. Validate the complete host sample.
        assert!(metrics.cpu_usage_pct.is_some());
        assert!(metrics.cpu_iowait_pct.is_none());
        let caps = collector.capabilities();
        assert!(!caps.cpu_iowait, "macOS iowait must be unsupported");

        // 8. Assert memory total and logical cores are nonzero.
        assert!(
            metrics.memory.total_bytes > 0,
            "memory total must be nonzero"
        );
        assert!(metrics.logical_cores > 0, "logical cores must be nonzero");

        // 9. Assert swap used is not greater than swap total.
        if let Some(swap) = &metrics.swap {
            assert!(
