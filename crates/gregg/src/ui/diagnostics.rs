...
pub fn render_key_hint(f: &mut Frame, area: Rect, state: &AppState) {
    ...
    let hint = if let Some(error) = state.config_reload_error.as_deref() {
        ...
        if state.active_pane == Pane::Eggpool {
            ...
            if both {
                "j/k select  h/l pane  v view  d drives  n net  q quit"
            } else {
                "j/k select  v view  d drives  n net  q quit"
            }
        } else if both {
            "j/k:select  h/l:pane  v:view  d:drives  n:net  Ctrl-R:refresh  q:quit"
        } else {
            "j/k:select  v:view  d:drives  n:net  Ctrl-R:refresh  q:quit"
        }
    };
