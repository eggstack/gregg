...
| `source` | `source.rs:1-448` | `FileSource` trait, `ProcSource` (prod), `MemorySource` (test) |
...
| `drives` | `drives.rs:1-230` | `/proc/self/mountinfo` + `statvfs` |
...
| `ffi` | `ffi.rs:1-1154` | Mach FFI, sysctl, RAII HostPort, `MacNativeQueries` trait |
| `cpu` | `cpu.rs:1-167` | Mach CPU tick deltas |
| `memory` | `memory.rs:1-147` | VM page counts → memory metrics |
| `swap` | `swap.rs:1-98` | sysctl `vm.swapusage` |
| `identity` | `identity.rs:1-86` | sysctl + SystemVersion.plist |
...
| `source` | `source.rs:1-908` | `WindowsSource` trait, `NativeWindowsSource` (prod), `Mock` |
