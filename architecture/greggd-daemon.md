...
| `update` | `src/update.rs` | Thin daemon lifecycle coordinator over the shared `gregg-update` mechanism: binds daemon identity, prepares the candidate via `prepare_candidate`, quiesces a running Windows SCM service only after full preparation, replaces, then restarts through exact-executable-aware manager ownership with `UpdatedButRestartFailed` partial-success; preserves the Plan 102 prepare-before-quiesce transaction rule |
