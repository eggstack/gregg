...
- for a non-root same-scope `greggd` replacement, captures valid default-config health before overwriting and, only when previously running, performs the new binary's config-specific `stop` + `croncheck` after shared finalization; first installs and stopped daemons remain stopped, and activation failure after replacement is nonzero with an exact retry command.
...
- Permission probe before any `greggd` shutdown; on `PermissionDenied` prints a shared platform-correct rerun hint (`sudo <exe> update` on Unix, Administrator terminal/PowerShell guidance on Windows) and returns 4 without stopping daemon; no internal elevation.
- `greggd` fully prepares and verifies the candidate before stopping a running Windows SCM service; stop failure is a hard pre-replacement error. It preserves config/registration and restarts only when running/managed, but manager mutation requires the registration's exact executable target; foreign/unknown ownership fails closed and Windows has no direct fallback for missing SCM. Stopped services remain stopped; successful replacement + failed restart is `UpdatedButRestartFailed` with installed version and exact retry command and nonzero exit. Direct/cron restart proves endpoint absence before spawning and waits for valid Gregg health readiness before success.
