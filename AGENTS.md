...
- The production event loop consumes owned `PollBatch` payloads; the borrowed `AppState::apply_batch(&PollBatch)` and borrowed normalization constructors remain compatibility paths. Ordered results use positional matching with stable-ID fallback, and endpoint host/port validation still gates every mutation.
...
- The event loop redraws only after render-visible transitions; unmapped input and no-op channel wakeups do not draw. Normal metric memoization is renderer-internal, keyed by stable system ID plus a compact render key, with index-aligned per-render access; condensed values are preformatted once per render.
...
- `startup install` defaults `auto` (Windows→SCM, macOS→launchd, Linux systemd-if-running else cron); systemd/launchd hosts never silently fall back to cron — print exact `sudo <exe> startup install --method …` and return `PermissionDenied`. No internal `sudo`. `restart` is exact-executable-aware: only an owned systemd/launchd/SCM registration may receive manager mutation; foreign same-config or unknown Unix ownership fails closed, foreign different-known-config registrations permit only the selected config's direct path, and Windows has no direct fallback for missing/foreign/unknown SCM. Otherwise Unix uses config-specific stop+absence-check+detached `run` and requires a valid health response, not just process spawn.
 - `gregg update` / `greggd update` are binary-first, crates.io-authoritative (`curl` max-stable SemVer compare, GitHub `latest` never authoritative), exact `vX.Y.Z` asset `<program>-<target>[.exe]`+`.sha256`, bounded `curl -fsSL --max-time` to owner-private `TempDir`, `sha2`-verified before chmod/exec, candidate `version` must equal `"<program> X.Y.Z"`, staged before touching current exe (`self-replace`), only HTTP 404 falls back to `cargo install --locked --version "=X.Y.Z"` (Plan 126 measured an eggfetch in-process candidate and closed RETAIN CURL: +105% stripped `greggd`, so external `curl` stays). `greggd` prepares fully before observing exact-executable lifecycle and before any stop (Unix: systemd/launchd ownership + selected-config health after preparation; Windows: `query_registration()` ownership revalidated immediately before quiescence, owned running/start-pending only may stop, owned stop-pending waits stopped without restart, foreign/unknown/`NotInstalled` perform zero SCM mutation, owned-to-foreign transition fails before replacement); restarts only `ManagedRunning`/`DirectRunning` via ownership-aware `restart_daemon()`, stopped/foreign remain stopped/preserved without fabricated restart claims. Shared permission rerun guidance is platform-correct: Unix uses `sudo`, Windows names an Administrator terminal/PowerShell and never emits `sudo`. Shared mechanics live in `gregg-update` (`UpdateSpec`-parameterized); `greggd` owns activation/restart.
...
- `greggd` HTTP transport is EggServe's direct H1 runtime (`eggserve-server` + `eggserve-primitives`, no core/static/TLS/H2/H3 compatibility crates). `ServerControl` and `ServerCompletion` remain separate under daemon supervision; set runtime limits explicitly, keep the total connection lifetime disabled, and serve cached `Bytes` through a known-length stream without payload copies. The current transport bounds request bodies at 64 KiB and retains GET/HEAD route behavior.
