...
- `gregg uninstall` / `greggd uninstall` (`--dry-run`, `--purge`) remove only the exact invoked executable (never a directory; sibling survives) plus daemon-owned startup integration actually present (systemd unit / launchd plist / managed cron block / `greggd` SCM `unregister`, each discovered independently); config preserved by default, `--purge` removes only resolved component files (never an arbitrary explicit parent); preflight before teardown, never internal `sudo`, uncertain direct stop blocks deletion, no `--all`, no receipt. Cargo-owned installs delegate on Unix / print the exact `cargo uninstall --root` handoff on Windows (never pathname-guessed, never parsed metadata).
