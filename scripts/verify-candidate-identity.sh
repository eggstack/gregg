...
if [[ "$MODE" == "pre-tag" ]]; then
    ...
    TAGGER_TIMESTAMP_ORIGINAL="$(git for-each-ref --format='%(taggerdate:iso8601-strict)' "refs/tags/${INPUT}")"
    # Normalize the tagger timestamp to canonical RFC3339 UTC ending in Z.
    TAGGER_TIMESTAMP="$(python3 -c '
import datetime as dt, sys
raw = sys.argv[1].strip()
if not raw:
    sys.exit(1)
# iso8601-strict may carry a +HH:MM or -HH:MM offset.
try:
    parsed = dt.datetime.fromisoformat(raw)
except ValueError:
    # Git may emit a space-separated form; normalize.
...
' "$TAGGER_TIMESTAMP_ORIGINAL")"
    ...
    # Validate tag metadata completeness (H2).
    [[ -n "$TAG_OBJECT_SHA" ]] || die "tag object SHA is empty"
    [[ -n "$TAGGER_NAME" ]] || die "tagger name is empty"
    [[ -n "$TAGGER_EMAIL" ]] || die "tagger email is empty"
    [[ -n "$TAGGER_TIMESTAMP" ]] || die "tagger timestamp is empty"
    [[ -n "$TAG_OBJECT_CONTENT_SHA" ]] || die "tag object content SHA is empty"
    [[ "$TAGGER_TIMESTAMP" == *Z ]] || die "tagger timestamp must end in Z"
else
    usage
fi
...
if [[ -n "$OUTPUT" ]]; then
    jq -n \
        ...
        --arg tagger_timestamp "${TAGGER_TIMESTAMP:-}" --arg tagger_timestamp_original "${TAGGER_TIMESTAMP_ORIGINAL:-}" \
        --arg tag_object_content_sha "$TAG_OBJECT_CONTENT_SHA" \
        ...
        '{mode:$mode,ref_input:$ref_input,candidate_sha:$candidate_sha,
          ...
          peeled_commit_sha:$peeled_commit_sha,head_sha:(if $head_sha == "" then null else $head_sha end),
          ...
          tagger_timestamp_original:(if $tagger_timestamp_original == "" then null else $tagger_timestamp_original end),
    ...
    if [[ -n "$TAG_OBJECT_SHA" ]]; then
        ...
        printf 'tagger_name=%s\ntagger_email=%s\ntagger_timestamp=%s\ntagger_timestamp_original=%s\ntag_object_content_sha256=%s\n' \
            "$TAGGER_NAME" "$TAGGER_EMAIL" "$TAGGER_TIMESTAMP" "$TAGGER_TIMESTAMP_ORIGINAL" "$TAG_OBJECT_CONTENT_SHA"
    fi
fi
