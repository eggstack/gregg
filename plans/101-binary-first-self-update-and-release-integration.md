...
2. crates.io metadata field/selection policy used;
3. SemVer implementation/dependency and measured rationale;
4. download transport choice for each binary;
5. checksum implementation choice;
6. final Unix replacement algorithm;
7. final Windows replacement algorithm and native smoke result;
8. Cargo fallback staging command;
9. `greggd` restart-state policy;
10. local release/update smoke results;
11. existing CI/release workflow run IDs used for native proof;
12. first real release where `gregg update`/`greggd update` successfully consume the Plan 099 assets.
